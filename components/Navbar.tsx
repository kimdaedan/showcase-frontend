  ...
  return (
    <nav className="w-full bg-white border-b border-gray-200 relative z-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Menggunakan Grid 3 Kolom agar menu benar-benar di tengah */}
        <div className="grid grid-cols-3 items-center h-20">
          ...
          {/* KOLOM TENGAH: Menu Navigasi (Centered) */}
          <div className="flex justify-center items-center space-x-10">

            {/* Menu EXHIBITION */}
            <Link href="/exhibition" className="group flex flex-col items-center text-center">
              <span className="text-sm font-bold text-gray-900 uppercase tracking-wider group-hover:text-blue-600 transition-colors">
                EXHIBITION
              </span>
            ...
            {/* Dropdown UPLOAD KARYA (Hover Trigger) */}
            <div
            ...
            >
              <button className="group flex flex-col items-center text-center focus:outline-none">
                <div className="flex items-center">
                  ...
                  <svg className={`ml-1 h-3 w-3 text-gray-400 transform transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-0 w-48 bg-white border border-gray-100 rounded-md shadow-xl py-2 z-50">
                  ...
                  <Link
                    href="/upload"
                    className="block px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors border-b border-gray-50"
                  >
                    Upload Karya Baru
                  </Link>
                  <Link
          ...
          {/* KOLOM KANAN: Login & Guest */}
          <div className="flex justify-end items-center space-x-6">
            {isLoggedIn ? (
              <button
                onClick={handleLogout}
                className="bg-red-600 text-white px-5 py-2 rounded shadow-md text-xs font-bold hover:bg-red-700 transition-all transform hover:scale-105 uppercase tracking-wide"
              >
                LOGOUT
              </button>
            ) : (
              <Link
                href="/login"
                className="bg-[#1e2329] text-white px-5 py-2 rounded shadow-md text-xs font-bold hover:bg-gray-800 transition-all transform hover:scale-105 uppercase tracking-wide"
              >
                LOGIN
              </Link>
            )}
